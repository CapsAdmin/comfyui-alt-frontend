...
export const BuildWorkflow = (fn: () => void) => {
    ...
    const seed = cyrb53(JSON.stringify(res))

    for (const node of res.nodes) {
        const hash = seed + node.id

        res.output[hash] = res.output[node.id]
        delete res.output[node.id]

        node.id = hash

        for (const key in node) {
            if (node[key] instanceof Array) {
                const [id, port] = node[key]
                if (typeof id === "string" && typeof port === "number") {
                    node[key] = [(seed + parseInt(id, 10)).toString(), port]
                }
            }
        }
    }
    ...
    for (const nodeId in res.output) {
        for (const key in res.output[nodeId].inputs) {
            const field = res.output[nodeId].inputs[key]
            ...
            if (field instanceof Array) {
                const [id, port] = field
                if (typeof id === "string" && typeof port === "number") {
                    res.output[nodeId].inputs[key] = [(seed + parseInt(id, 10)).toString(), port]
                }
            }
        }
    }
